...
const int ProcessRangeIfPriceLeftInPoints = 0;
const double Lots = 0.05;
...
void OnTick() {
    ...
    if (curRangeBottom != prevRangeBottom) {
        if (prevRangeBottom != 0) {
            processOldRange(prevRangeBottom, false, true);
        }
    ...
    for (int i = 0; i < ArraySize(allRanges); i++) {
        int rangeBottom = allRanges[i];
        int rangeTop = rangeBottom + RangeSizeInPoints;
        if (rangeBottom == curRangeBottom) {
            continue;
        }
        int priceInPoints = (int)((Ask + Bid)/2 / _Point);
        if (
            rangeBottom - priceInPoints > ProcessRangeIfPriceLeftInPoints ||
            priceInPoints - rangeTop > ProcessRangeIfPriceLeftInPoints
        ) {
            processOldRange(rangeBottom, true, false);
        }
    }
}
...
void getAllRanges(int &allRanges[]) {
    for (int pos = 0; pos < OrdersTotal(); pos++) {
        ...
        int rangeBottom = getRangeBottom(OrderOpenPrice());
        if (!existInArray(rangeBottom, allRanges)) {
            appendArray(rangeBottom, allRanges);
        }
    }
}

void processOldRange(int rangeBottom, bool shouldCloseOpenOrders, bool shouldCloseWaitingOrders) {
